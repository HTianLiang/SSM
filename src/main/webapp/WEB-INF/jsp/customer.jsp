...
<body>
			...
			<!-- 新增客户信息 -->
				<div class="panel-group">
					<div class="center-block text-center">
						<td>
							<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#customerAddDialog" onclick="addCustomer()">新增客户信息</a>
						</td>
					</div>
					<!-- /.col-lg-12 -->
				</div>
							...
							<tbody>
								<c:forEach items="${page.rows}" var="row">
									<tr>
										...
										<td>
										<c:forEach items="${fromType}" var="item">
											<c:if test="${item.dict_id == row.cust_source}">${item.dict_item_name}</c:if>
										</c:forEach>
										</td>
										<%--<td>${row.cust_industry}</td>--%>
										<td>
											<c:forEach items="${industryType}" var="item">
												<c:if test="${item.dict_id == row.cust_industry}">${item.dict_item_name}</c:if>
											</c:forEach>
										</td>
										<%--<td>${row.cust_level}</td>--%>
										<td>
											<c:forEach items="${levelType}" var="item">
												<c:if test="${item.dict_id == row.cust_level}">${item.dict_item_name}</c:if>
											</c:forEach>
										</td>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabeladd">新增客户信息</h4>
				</div>
				<div class="modal-body">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary" onclick="updateAddCustomer()">保存</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		function updateAddCustomer() {
			$.post("<%=basePath%>customer/updateAdd.action",$("#add_customer_form").serialize(),function(data){
				alert("客户信息新增成功！");
				window.location.reload();
			});
		}
